      ...
      foreach ($text_nodes as $original_node) {
        ...
        if ($match_count > 0) {
          ...
          foreach ($matches[0] as $delta => $match) {
            ...
            if ($word) {
              ...
              if (!isset($word_link_rendered[$word_id])) {
                if ($cache = \Drupal::cache()->get('word_link_rendered_' . $word_id)) {
                  ...
                  $lnk = Link::fromTextAndUrl($word->name->value, $url);
              ...
              $link->appendXML($word_link_rendered[$word_id]);
            ...
            if ($word->page_max_links->value > 0 && $word->page_max_links->value == $word_match_count) {
              return Html::serialize($dom);
            }

            $page_links_count++;
            if ($page_links_count == $settings->get('word_link_page_max_links')) {
              return Html::serialize($dom);
            }
          }
        }
